...
# Важная информация о предположениях модели
with st.expander("ℹ️ Предположения модели и методология расчёта"):
    st.markdown("""
    **Модель основана на следующих ключевых предположениях:**
    
    1. **Стоимость вакансии (в день)**
       - Используется для оценки потерь от задержки нового сотрудника
       - Примерно 20-30% от среднего дневного заработка позиции
       - Включает: потерю производительности, найм и адаптацию
    
    2. **Стоимость ошибки найма**
       - Включает: зарплату, период адаптации, текучесть, переподготовку
       - Для инженеров: 400-500k ₽, для других специалистов: ниже
    
    3. **Сокращение Time-to-Hire (TtH)**
       - От контрольной проверки можно ускорить на 2-8 дней в зависимости от процесса
       - Экономия считается за каждого нанятого сотрудника отдельно
    
    **Вы можете:**
    - ✏️ Отредактировать любой параметр в левой панели
    - 🔗 Поделиться результатами через URL (все параметры кодируются)
    - 📥 Скачать все параметры и результаты в CSV
    """)
